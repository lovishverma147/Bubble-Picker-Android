...
import com.lovish.bubblepicker.physics.CircleBody
import com.lovish.bubblepicker.rendering.BubbleShader.U_MATRIX
import com.lovish.bubblepicker.toTexture
...
data class Item(val pickerItem: PickerItem, val circleBody: CircleBody) {
    ...
    private fun placeText(paint: TextPaint): StaticLayout? {
        return pickerItem.title?.let {
            StaticLayout.Builder.obtain(
                it,
                0,
                pickerItem.title!!.length,
                paint,
                (bitmapSize * 0.9).toInt()
            )
